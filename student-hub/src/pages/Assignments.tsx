...
export default function Assignments() {
  ...
  const fetchAssignments = async () => {
    try {
      ...
      const assignmentsPromises = allClasses.map((classItem: any) =>
        api.get(`/assignments/class/${classItem._id}`)
      );
  ...
  const handleGradeAssignment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: error.response?.data?.message || "Failed to grade assignment",
        variant: "destructive",
      });
    } finally {
      setFormLoading(false);
  ...
  // Improved function to show student's submission details
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              {user?.role === "student" ? "View and submit assignments" : "Manage assignments"}
            </p>
          </div>
        ...
        {assignments.length === 0 ? (
          ...
          <div className="grid gap-6">
            {assignments.map((assignment) => {
              ...
              return (
                <Card key={assignment._id} className="transition-all hover:shadow-lg">
