...
import { Plus, Brain, Clock, Award, Loader2, Play, CheckCircle2, Trash2, Eye, X } from "lucide-react";
...
export default function Quizzes() {
    ...
    try {
      ...
      if (user?.role === "teacher") {
        ...
        const quizzesPromises = allClasses.map((classItem: any) =>
          api.get(`/quizzes/class/${classItem._id}`)
        );
  ...
  const handleReviewQuiz = async (quiz: Quiz) => {
    try {
      const response = await api.get(`/quizzes/${quiz._id}/submission`);
  ...
  const handleViewStudentSubmission = async (quiz: Quiz, studentId: string) => {
    try {
      const response = await api.get(`/quizzes/${quiz._id}/submissions/${studentId}`);
      ...
      <div className="space-y-6">
        ...
        {quizzes.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {quizzes.map((quiz) => (
              <Card key={quiz._id} className="transition-all hover:shadow-lg">
                ...
                <CardContent className="space-y-4 pt-0">
                  ...
                  {(user?.role === "teacher" || user?.role === "admin") && isQuizOwner(quiz) && (
                    <div className="flex gap-2">
                      ...
                      >
                        <Award className="h-4 w-4" />
                        View Results ({quiz.submissions?.length || 0})
                      </Button>
                    </div>
                  )}
                </CardContent>
                  ...
                  <TableBody>
                    {studentSubmissions.map((submission) => (
                      <TableRow key={submission._id}>
                        ...
                        <TableCell>
                          {typeof submission.student === 'object' ? submission.student.email : 'N/A'}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleViewStudentSubmission(selectedQuiz, 
                              typeof submission.student === 'object' ? submission.student._id : submission.student)}
                          >
